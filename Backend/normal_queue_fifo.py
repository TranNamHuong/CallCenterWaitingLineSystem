...
class NormalQueue:
    ...
    def remove_by_id(self, customer_id: str) -> bool:
        """Cách xoá đáng tin cậy — dùng ID duy nhất, không lo trùng tên."""
    ...
    def remove(self, name: str) -> bool:
        """CẢNH BÁO: xoá theo tên — nếu có 2 khách trùng tên, chỉ xoá người
        xuất hiện TRƯỚC trong hàng đợi (có thể không phải người bạn muốn xoá).
        Ưu tiên dùng remove_by_id() khi có customer_id."""
        for customer in self._queue:
    ...
    def get_wait_position_by_id(self, customer_id: str) -> int:
        """Cách tra vị trí đáng tin cậy — dùng ID duy nhất."""
    ...
    def get_wait_position(self, name: str) -> int:
        """CẢNH BÁO: tra theo tên — nếu trùng tên sẽ trả về vị trí của người
        xuất hiện TRƯỚC, không phân biệt được các khách trùng tên."""
        for i, customer in enumerate(self._queue):
            if customer.name == name:
                return i + 1
        return -1
...
def serve_next_customer(queue: NormalQueue) -> dict:
    ...
    return {"success": False, "message": "Hàng đợi đang trống. Không có khách nào cần phục vụ."}
